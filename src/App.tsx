...
import { ArrowDownWideNarrow, ArrowLeft, ArrowRight, ArrowUpRight, BookOpen, Bookmark, Check, ChevronDown, Code2, ExternalLink, FileText, Github, Grid2X2, Layers3, Library, List, Menu, Search, Sparkles, X } from 'lucide-react'
...
export default function App() {
  ...
  const visible = papers.filter(p => (view !== 'saved' || saved.includes(p.id)) &&
    ...
    .sort((a, b) => sort === 'oldest' ? a.published.localeCompare(b.published) : b.published.localeCompare(a.published))
  const featured = papers.find(p => p.featured)
  const latest = papers.map(p => p.published).sort().at(-1)
